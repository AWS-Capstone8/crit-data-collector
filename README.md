...
```json
{
  "collected_at": "2026-05-14T07:45:22+00:00",
  "total_channels": 6155,
  "total_videos": 296379,
  "groups": 113,
  "tables": {
  ...
  "category_timestamps": {
    "1": "2026-05-14T07:25:13+00:00",
    "20": "2026-05-14T07:37:28+00:00",
    "28": "2026-05-14T07:45:22+00:00"
  }
}
```

- `total_channels` / `total_videos`: 누적 합산값 (매주 수집 시 기존 값에 추가)
- `tables`: 해당 카테고리 수집 시 최신 데이터로 덮어쓰기 (병합)
- `category_timestamps`: 카테고리별 마지막 수집 시각
...
| 범위 | 채널 | 영상 | 쿼터 | 소요 시간 |
|---|---|---|---|---|
| 5개 카테고리 (그룹1) | ~1,000 | ~50,000 | ~3,300유닛 | ~12분 |
| 14개 카테고리 전체 | ~2,300 | ~110,000 | ~7,000유닛 (70%) | ~20분 |

> 카테고리당 최대 200개 채널이 발견됨 (mostPopular 4페이지 × 50개 영상에서 유니크 채널 추출)
...
└── logs/
    └── {YYYYMMDD_HHMMSS}.json ← 실행 로그 (채널수, 영상수, 쿼터 소모)
```
